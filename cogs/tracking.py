...
class TrackingCog(commands.Cog):
    ...
    valorant = app_commands.Group(
        name="valorant",
        description="Valorant rank (Tracker.gg if configured, else Henrik)",
    )
    ...
    async def val_rank(self, interaction: discord.Interaction, member: discord.Member | None = None):
        ...
        except (HenrikError, TrackerGGError) as e:
            await interaction.followup.send(f"Valorant API error ({e.status}): {e}", ephemeral=True)
            return
